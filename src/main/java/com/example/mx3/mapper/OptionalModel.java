 ...
 * @since 2016-11-06
 */
public abstract class OptionalModel<T extends OptionalModel<?>> implements Serializable {
    ...
    public Optional<T> selectById(Serializable id) {
        SqlSession sqlSession = sqlSession();
        try {
            T t = sqlSession.selectOne(sqlStatement(SqlMethod.SELECT_BY_ID), id);
     ...
     * @param queryWrapper 实体对象封装操作类（可以为 null）
     */
    public Optional<T> selectOne(Wrapper<T> queryWrapper) {
        T t = SqlHelper.getObject(() -> LogFactory.getLog(this.entityClass), selectList(queryWrapper));
        return Optional.ofNullable(t);
    }
